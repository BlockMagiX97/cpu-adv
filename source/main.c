...
static const char *reg_names[41] = {
    ...
    "PC","SP1","FR","SP0","PPTR","IMR","ITR","SLR","PPR"
};

static const char *opcode_names[20] = {
    "MOV","ADD","SUB","MUL","DIV","OR","AND","NOT","XOR",
    "PUSH","POP","CALL","CMP","CMOV","RET","RETI","SYSRET",
    "SYSCALL","HLT","COANDSW"
};

static const char *cmov_names[8] = {
    [NE] = "NE", [GT] = "GT", [LT] = "LT",
    [EQ] = "EQ", [LE] = "LE", [GE] = "GE"
};

static void format_inst(struct core *,
                       struct instruction *inst,
                       uint64_t pc,
                       char *buf,
                       size_t buf_sz)
{
    int off = snprintf(buf, buf_sz, "%016" PRIx64 ": %s",
    ...
    case RR:
        snprintf(p, rem, " %s, %s",
                 reg_names[inst->register_register.reg1],
    ...
    case RI:
        snprintf(p, rem, " %s, 0x%016" PRIx64,
                 reg_names[inst->register_imm.reg1],
                 inst->register_imm.imm64);
        break;
    case OA:
        if (inst->one_arg.mode == REGISTER)
            snprintf(p, rem, " %s",
                     reg_names[inst->one_arg.reg]);
        else if (inst->one_arg.mode == ADDRESS)
            snprintf(p, rem, " [0x%016" PRIx64 "]",
                     inst->one_arg.address);
        else
            snprintf(p, rem, " 0x%016" PRIx64,
                     inst->one_arg.imm64);
        break;
    case CM:
        snprintf(p, rem, " %s %s, %s",
                 cmov_names[inst->cmove.cond],
                 reg_names[inst->cmove.reg1],
                 reg_names[inst->cmove.reg2]);
        break;
    case NO:
        break;
    }
...
int main(int argc, char **argv) {
    ...
    struct ram *memory = init_memory(1<<20);
    FILE *f = fopen(argv[1], "rb");
    if (!f) { perror("fopen"); return 1; }
    fread(memory->mem, 1, memory->cap, f);
    fclose(f);
    ...
    while (!halted) {
        ...
        for (int i = 0; i < rows; i++) {
            ...
            char buf[128];
            format_inst(&cpu, &inst, cur, buf, sizeof buf);

            if (cur == pc) wattron(w_inst, A_REVERSE);
            mvwprintw(w_inst,1+i,1,"%.*s", col_w-2, buf);
            if (cur == pc) wattroff(w_inst, A_REVERSE);
        ...
        int rw   = col_w - 2;
        int cols = rw / 20; if (cols < 1) cols = 1;
        ...
        for (int c = 0; c < cols; c++) {
            for (int r = 0; r < per; r++) {
                ...
                int y = 1 + r, x = 1 + c*20;
                if (idx == PC) wattron(w_regs, A_BOLD);
                mvwprintw(w_regs,y,x,"%-4s:%016" PRIx64,
                          reg_names[idx], v);
                if (idx == PC) wattroff(w_regs, A_BOLD);
            }
        }
        wrefresh(w_regs);
        ...
        int mbw     = col_w - 10;
        uint64_t bpr     = mbw/3; if (bpr<1) bpr=1;
        int mrows   = H - 2;
        ...
        for (int i = 0; i < srows; i++) {
            ...
            mvwprintw(w_stack,1+i,1,"%016" PRIx64 ":%016" PRIx64,
                      a, v);
        }
        wrefresh(w_stack);
        ...
        for (int i = 0; i < prow; i++) {
            ...
            mvwprintw(w_page,1+i,1,
                        "%016" PRIx64 " -> %016" PRIxPTR,
                        va, pa);
        }
        wrefresh(w_page);
